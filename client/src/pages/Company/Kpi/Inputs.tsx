...
type Props = {
    ...
    toggle: string;
    id: string;
    handleDelete:({id,index}:{id:string,index:string|number})=>void
};

const Inputs = (props: Props) => {
    const { register, index, errors, id, toggle,handleDelete } = props;
    ...
    return (
        <div className={`${id === toggle ? '' : 'hidden'} m-5 p-5 border rounded-lg bg-gray-100`}>
         <CustomInput<IForm> title={"Kpi Name"} type={"text"} rules={textValidation("Kpi Name")} name={`kpis.${index}.kpiName`} placeholder={"Enter Kpi Name"} errors={errors?.kpis?.[index]?.kpiName} register={register}/>
            ...
            <div>
                {index === 0 ? '' : <button type="button" onClick={()=>{ handleDelete({id,index})}} className="bg-red-500 text-white px-3 py-1 rounded-md">Delete</button>}
            </div>
        </div>
    );
};
