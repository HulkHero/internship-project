...
const SideNav = ({sideNav}:Props) => {
  ...
  return (
    ...
    {user.systemRole==="admin"&&<NavLink to="/company/"  className={({isActive})=> isActive? 'bg-darkRed pl-2 py-1 rounded-lg rounded-r-none text-md font-semibold opacity-95': ' hover:bg-black hover:bg-opacity-20 pl-2 py-1 rounded-lg rounded-r-none text-md font-semibold opacity-80 hover:opacity-95'}>Dashboard</NavLink>}
